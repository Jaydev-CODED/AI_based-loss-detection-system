...
import { productTrackingData } from "@/data/mockData";
import { MapPin, Search, PackagePlus, ArrowRightLeft, Truck, AlertTriangle } from "lucide-react";
...
const ProductTrackingTab = () => {
  const [selectedProduct, setSelectedProduct] = useState(productTrackingData[0].productId);
  const [search, setSearch] = useState("");

  const product = productTrackingData.find((p) => p.productId === selectedProduct)!;
        ...
        <div className="glass rounded-xl p-4 space-y-3">
          ...
          {filteredProducts.map((p) => (
            ...
            >
              ...
              <p className={`text-xs mt-0.5 ${selectedProduct === p.productId ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
